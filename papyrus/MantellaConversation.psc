...
function ContinueConversation(int handle)
    ...
    if(nextAction == mConsts.KEY_REPLYTTYPE_STARTCONVERSATIONCOMPLETED)
        ...
        if F4SE_HTTP.hasKey(handle, mConsts.KEY_QUEST_IDS_TO_CHECK)
            _questIdsToCheck = F4SE_HTTP.getIntArray(handle, mConsts.KEY_QUEST_IDS_TO_CHECK)
        else
...
function NpcSpeak(Actor actorSpeaking, string lineToSay, Topic topicToUse, bool isSpokenByNarrator, float duration, Actor actorToSpeakTo)
    ...
    if ret != 0
        Debug.Trace("Patcher returned " + ret);                              ; Logged only, no in-game notification
    Endif
...
int Function BuildCustomContextValues()
    ...
    ; Quest awareness: send quest stages back to Python
    if _questIdsToCheck && _questIdsToCheck.Length > 0
        string questData = BuildQuestData()
        if questData != ""
            F4SE_HTTP.setString(handleCustomContextValues, mConsts.KEY_CONTEXT_NPC_QUESTS, questData)
        endif
    endif
...
string function BuildQuestData()
    ; Build "QuestName:status[:stage]|..." string for Python
    string result = ""
    int i = 0
    while i < _questIdsToCheck.Length
        Quest q = Game.GetForm(_questIdsToCheck[i]) as Quest
        if q
