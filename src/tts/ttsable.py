...
class TTSable(ABC):
    ...
    def synthesize(self, voice: str, voiceline: str, in_game_voice: str, csv_in_game_voice: str, voice_accent: str, synth_options: SynthesizationOptions, advanced_voice_model: str | None = None):
        ...
        known_voices = {v.lower() for v in (voice, in_game_voice, csv_in_game_voice, advanced_voice_model, f'fo4_{voice}') if isinstance(v, str) and v}
        if self._last_voice == '' or self._last_voice.lower() not in known_voices:
        ...
        if (os.path.exists(final_voiceline_file)):
            ...
            new_name: str = "".join(c for c in unique_name if c.isprintable() and c not in r'\/:*?"<>|.')
